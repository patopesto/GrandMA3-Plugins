---
title: ViewScaler
description: &desc Scale a display's content to fit the available space
template: splash
prev: true
next: true
hero:
  tagline: *desc
  image:
    file: '@assets/ViewScaler_menu.png'
    alt: ViewScaler UI menu
  image_attrs:
    style: 'rotate: 3deg; filter: drop-shadow(0px 0px 75px rgba(255, 255, 255, 0.2));'
  actions:
    - text: Download
      link: '#installation'
    - text: Help
      link: '#usage'
      variant: secondary
    - text: Release Notes
      link: '#release-notes'
      variant: secondary
plugin:
  version: v0.2.0.0
  grandma3: v2.0.2.0
---

import { Badge, Aside } from '@astrojs/starlight/components';
import Timeline from '@components/Timeline.astro';
import Installation from './_components/Installation.mdx';


<div style="text-align: center">
This plugin came as a need to handle the views on different screen sizes when using multiple platforms: onPC, Compact(XT) or Lite/Full consoles.
</div>

<div style="width: 80%; text-align: center; margin: 3rem auto 3rem auto;">
![ViewScaler Demo Video](@assets/ViewScaler_demo.gif)
</div>

---

## Usage

When called, the plugin will scale the content of the currently focused display.

```lua
Plugin "ViewScaler"
```

### Arguments:

- `/Screen X`: Run on specified display

### Examples:

- Run on Display 3

  ```lua
  Plugin "ViewScaler" "/Screen 3"
  ```

## Notes

<Aside>
A display's content is not linked to the `Views` DataPool object which might have been called previously. This plugin will only affect the display's current content, not the original `View` object.
</Aside>

<Aside>
This plugin does not affect the display width, height or scale settings. It rearranges the position and size of the widgets currently visible on screen.
</Aside>

### Screen Grid Sizes

The display grid sizes for the most common configurations

| Platform                       | Width | Height | Notes
| :----------------------------- | :---: | :----: | :----
| Compact (XT)                   | 18    | 7      |
| Lite / Full                    | 18    | 10     |
| External 24" (Display 5 and 6) | 18    | 10     | with scale x1
| onPC MacBookPro 14"            | 18    | 9      | with scale x0.75
| onPC MacBookPro 16"            | 21    | 11     | with scale x0.75

---


## Installation

Requires **grandMA3 version** <Badge text={frontmatter.plugin.grandma3} size="medium" variant="caution"/> or above.

<Installation folder={frontmatter.title}/>


## Release Notes

<Timeline>
- #### Release v0.2.0.0 - 23/08/2025 <Badge text="Latest" size="large" variant="tip" style="margin-left: 15px;"/>
  - Made screen configurable with argument `/Screen X`

- #### Release v0.1.0.1 - 18/08/2025
  - Fixed packaging issue

- #### Release v0.1.0.0 - 18/08/2025 
  - Initial release
</Timeline>
